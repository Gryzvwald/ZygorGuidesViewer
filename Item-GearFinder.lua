...
function GearFinder:CreateSystemTab()
	...
	if self.BlizzardTab then return end
	...
	if _G["CharacterFrameTab"..n-1]:IsVisible() then -- currency tab is not visible on low level chars, so we need to attach either to it or to previous one
		tab:SetPoint("LEFT", _G["CharacterFrameTab"..n-1], "RIGHT", -16, 0)
	else
		tab:SetPoint("LEFT", _G["CharacterFrameTab"..n-2], "RIGHT", -16, 0)
	end
	tab:Show()
